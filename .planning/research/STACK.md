...
**Domain:** Multi-AI "debate council" backend (FastAPI + LangGraph on Cloud Run) with Supabase-realtime PWA frontend, zero-cost LLM APIs
**Researched:** 2026-08-20
**Confidence:** HIGH (Cloud Run billing model, LangGraph Send API, Supabase JS/Py clients, OpenRouter rate limits — all verified against official docs) / MEDIUM (exact free-model roster, Gemini free-tier numeric quotas — these rotate frequently, verify at implementation time)
...
```bash
# Core
pip install "fastapi==0.141.*" "uvicorn[standard]==0.52.*" "langgraph==1.2.*" "langchain-core==1.6.*"

# Model providers
pip install "langchain-google-genai==4.3.*" "langchain-openai==1.6.*"

# Supabase (backend)
pip install "supabase==2.31.*"

# Config
pip install "python-dotenv==1.2.*" "pydantic==2.13.*"

# Dev dependencies
pip install ruff pytest pytest-asyncio
```

Frontend PWA: no `npm install` — reference `https://cdn.jsdelivr.net/npm/@supabase/supabase-js@2/+esm` directly from a `<script type="module">`, same as the other BONA PWAs.
...
**If running the debate synchronously inside `POST /debates` (recommended):**
...
**If you ever need true fire-and-forget background processing (future, not this milestone):**
...
**For the LangGraph graph shape itself:**
...
---
*Stack research for: bonasterio (multi-AI debate council, FastAPI+LangGraph+Cloud Run+Supabase+PWA)*
*Researched: 2026-08-20*
